...
TEST_CASE("verify: return value preserves expression value")
{
	AssertTest::reset();

	SECTION("disabled with true returns true") { CHECK(OKL_VERIFY(disabled, true)); }
	SECTION("disabled with false returns false") { CHECK_NOT(OKL_VERIFY(disabled, false)); }
	SECTION("disabled returns the int value, not just a bool") { CHECK(OKL_VERIFY(disabled, 42) == 42); }
	SECTION("enabled with truthy expression returns the truthy value")
	{
		CHECK(OKL_VERIFY(release, true));
		CHECK(OKL_VERIFY(release, 42) == 42);
	}
	SECTION("enabled with falsy expression returns the falsy value")
	{
		CHECK_NOT(OKL_VERIFY(release | non_fatal | log_always, false));
		CHECK(OKL_VERIFY(release | non_fatal | log_always, 0) == 0);
	}
	SECTION("non-null pointer returns the pointer")
	{
		constexpr int value{0};
		const int* ptr{&value};
		CHECK(OKL_VERIFY(release, ptr) == &value);
	}
	SECTION("null pointer returns nullptr")
	{
		const int* ptr{nullptr};
		CHECK(OKL_VERIFY(release | non_fatal | log_always, ptr) == nullptr);
	}
	SECTION("binary expression returns bool")
	{
		CHECK(OKL_VERIFY(release, 5 < 10));
		CHECK_NOT(OKL_VERIFY(release | non_fatal | log_always, 10 < 5));
	}
};
...
TEST_CASE("verify: usable as a boolean expression")
{
	CHECK(OKL_VERIFY(release, true));
	CHECK_NOT(OKL_VERIFY(release | non_fatal | log_always, false));
	CHECK(OKL_VERIFY(disabled, true));
	CHECK_NOT(OKL_VERIFY(disabled, false));
};
