...
CONSTEXPR_TEST_CASE(test_init)
{
	...
	REQUIRE(bitflag3.has_flags(ETestEnum::first));
	REQUIRE(bitflag3.has_flags(ETestEnum::second));
	REQUIRE(!bitflag3.has_flags(ETestEnum::third));

	OKL_WARNING_POP()
};

CONSTEXPR_TEST_CASE(test_assignment)
{
...
CONSTEXPR_TEST_CASE(test_conversion)
{
	constexpr TestBitflag bitflag{ETestEnum::first};
...
CONSTEXPR_TEST_CASE(test_valid_flags)
{
	REQUIRE(Okl::Bitflag<ETestEnum2>::num_valid_flags() == 3);
	REQUIRE(static_cast<TestBitflag::UnderlyingType>(Okl::Bitflag<ETestEnum2>::valid_flags) == 0b00101100u);

	REQUIRE(TestBitflag::num_valid_flags() == 6);
	REQUIRE(static_cast<TestBitflag::UnderlyingType>(TestBitflag::valid_flags) == 0b10101111u);
};

TEST_CASE(test_underlying)
{
	OKL_WARNING_PUSH()
	OKL_DISABLE_WARNING_MSVC(26813, "Use 'bitwise and' to check if a flag is set.")
	...
	REQUIRE(bitflag_mut.flags() == ETestEnum::second);

	OKL_WARNING_POP()

	constexpr TestBitflag bitflag_const{ETestEnum::third};
	REQUIRE(bitflag_const.underlying() == 4);
};

CONSTEXPR_TEST_CASE(test_has_flags)
{
	constexpr TestBitflag bitflag{ETestEnum::first};
	constexpr TestBitflag bitflag1{bitflag};
	REQUIRE(bitflag.has_flags(ETestEnum::first));
	REQUIRE(!bitflag1.has_flags(ETestEnum::second));
};

CONSTEXPR_TEST_CASE(test_has_any_flag)
{
	constexpr TestBitflag bitflag{ETestEnum::first};
	REQUIRE(bitflag.has_any_flag(TestBitflag{ETestEnum::first} | ETestEnum::third));
	REQUIRE(!bitflag.has_any_flag(TestBitflag{ETestEnum::third} | ETestEnum::fourth));
};

CONSTEXPR_TEST_CASE(test_add_flags)
{
	TestBitflag bitflag{ETestEnum::first};
	bitflag.add_flags(ETestEnum::second);
	REQUIRE(bitflag.has_flags(ETestEnum::first));
	REQUIRE(bitflag.has_flags(ETestEnum::second));
	...
	REQUIRE(bitflag.has_flags(ETestEnum::first));
	REQUIRE(bitflag.has_flags(ETestEnum::second));
	REQUIRE(bitflag.has_flags(ETestEnum::third));
	REQUIRE(bitflag.has_flags(ETestEnum::fourth));
	REQUIRE(!bitflag.has_flags(ETestEnum::sixth));
};

CONSTEXPR_TEST_CASE(test_remove_flags)
{
...
CONSTEXPR_TEST_CASE(test_toggle_flags)
{
	TestBitflag bitflag{TestBitflag{ETestEnum::first} | ETestEnum::second};
...
CONSTEXPR_TEST_CASE(test_set_flags_to)
{
	TestBitflag bitflag{TestBitflag{ETestEnum::first} | ETestEnum::second};
...
CONSTEXPR_TEST_CASE(test_clear_flags)
{
	TestBitflag bitflag{TestBitflag{ETestEnum::first} | ETestEnum::second};
	bitflag.clear_flags();
	REQUIRE(!bitflag.has_flags(ETestEnum::first));
...
CONSTEXPR_TEST_CASE(test_bitwise_ops)
{
	constexpr TestBitflag bitflag{ETestEnum::first};
	...
	REQUIRE((bitflag & bitflag1).m_flags == static_cast<ETestEnum>(0));
	REQUIRE((bitflag & ETestEnum::first).m_flags == ETestEnum::first);
	REQUIRE((ETestEnum::first & bitflag).m_flags == ETestEnum::first);

	REQUIRE((bitflag | bitflag1).m_flags ==
	        static_cast<ETestEnum>(static_cast<Okl::uint8>(ETestEnum::first) |
	                               static_cast<Okl::uint8>(ETestEnum::second)));
	REQUIRE((bitflag | ETestEnum::second).m_flags == (bitflag | bitflag1).m_flags);
	REQUIRE((ETestEnum::first | bitflag1).m_flags == (bitflag | bitflag1).m_flags);

	REQUIRE((bitflag ^ bitflag1).m_flags == (bitflag | bitflag1).m_flags);
	REQUIRE((bitflag ^ ETestEnum::first).m_flags == static_cast<ETestEnum>(0));
	REQUIRE((ETestEnum::first ^ bitflag).m_flags == static_cast<ETestEnum>(0));

	REQUIRE((~bitflag).m_flags == static_cast<ETestEnum>(~static_cast<TestEnumUnderlyingType>(ETestEnum::first)));

	OKL_WARNING_POP()
};

CONSTEXPR_TEST_CASE(test_comparison)
{
	constexpr TestBitflag bitflag{ETestEnum::first};
	constexpr TestBitflag bitflag1{ETestEnum::second};
	...
	REQUIRE(bitflag < bitflag1);
	REQUIRE(bitflag <= bitflag1);
	REQUIRE(bitflag1 > bitflag);
	REQUIRE(bitflag1 >= bitflag);

	REQUIRE(bitflag < ETestEnum::second);
	REQUIRE(bitflag1 > ETestEnum::first);
};
